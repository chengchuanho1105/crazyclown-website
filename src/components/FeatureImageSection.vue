...
const alignClass = computed(() =>
  props.align === 'right' ? 'right-0 items-end text-right' : 'left-0 items-start text-left',
)
const bgUrl = computed(() => props.bgImage || 'https://picsum.photos/1920/1080?random=1')
const roundedClass = computed(() =>
  props.align === 'right' ? 'rounded-none md:rounded-l-full' : 'rounded-none md:rounded-r-full',
)
...
<template>
  <section
    ref="sectionRef"
    class="relative w-full h-screen mb-20 flex items-center overflow-hidden"
  ...
  >
    ...
    >
      <h2
        class="text-3xl md:text-5xl font-extrabold mb-4 text-gray-900 dark:text-white drop-shadow"
      >
        {{ props.title }}
      </h2>
      <p
        class="mb-6 text-lg text-gray-700 dark:text-gray-200 leading-relaxed"
