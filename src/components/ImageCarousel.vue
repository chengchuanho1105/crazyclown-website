...
const swiperOptions = computed(() => ({
  ...
  autoplay: props.autoPlay
    ...
    : false,
  pagination:
    props.showIndicators && props.items.length > 1
      ? {
          clickable: true,
        }
      : false,
      ...
      <SwiperSlide v-for="(item, index) in items" :key="index">
        <div class="relative w-full h-full">
          ...
          <video
            ...
            :src="item.videoUrl"
            :type="item.videoType ? `video/${item.videoType}` : 'video/mp4'"
            autoplay
            muted
            loop
            playsinline
          ...
          <div class="absolute inset-0 flex items-center justify-center z-10">
            <div class="text-center text-white px-8 max-w-4xl mx-auto">
              <h2 v-if="item.title" class="text-4xl md:text-6xl font-extrabold mb-6 drop-shadow-lg">
                {{ item.title }}
              </h2>
              <p
                v-if="item.description"
                class="text-lg md:text-xl mb-8 text-gray-200 leading-relaxed max-w-2xl mx-auto"
