...
const logoUrl = computed(() => {
  try {
    ...
    if (
      ...
      'value' in brandLogo &&
      typeof brandLogo.value === 'string'
    )
      return brandLogo.value
    if (brandLogo && typeof brandLogo === 'object' && 'value' in brandLogo)
      return String(brandLogo.value)
  } catch {}
  return ''
})
</script>
...
<template>
  ...
  >
    <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-10 pb-8">
      ...
      <div class="flex flex-col items-start gap-3">
        <div class="flex items-center gap-3 mb-2">
          <img
            v-if="logoUrl && typeof logoUrl === 'string'"
            :src="logoUrl"
            :alt="brandDisplayName as string"
            class="w-12 h-12 rounded-xl shadow-md object-contain bg-white dark:bg-gray-900"
          />
          <a
            v-if="brandWebsiteUrl"
            :href="brandWebsiteUrl as string"
            class="text-2xl font-extrabold tracking-wide text-sky-700 dark:text-sky-200 hover:underline"
            target="_blank"
            rel="noopener"
          >
            {{ brandDisplayName }}
        ...
        <div class="text-sm text-gray-600 dark:text-gray-400 space-y-1">
          <div v-if="brandContactEmail">
            <i class="bi bi-envelope-fill mr-1"></i>Email：<a
              :href="`mailto:${brandContactEmail}`"
              class="hover:underline text-blue-700 dark:text-blue-300"
              >{{ brandContactEmail }}</a
            >
          </div>
          <div v-if="brandContactPhone">
            <i class="bi bi-telephone-fill mr-1"></i>電話：<a
              :href="`tel:${brandContactPhone}`"
              class="hover:underline text-blue-700 dark:text-blue-300"
              >{{ brandContactPhone }}</a
            >
          </div>
          <div v-if="brandContactAddress">
            <i class="bi bi-geo-alt-fill mr-1"></i>地址：{{ brandContactAddress }}
          </div>
        </div>
      </div>
        ...
        <div class="flex flex-wrap gap-3 mt-2">
          <template v-if="brandSocial">
            ...
            <div v-if="brandSocial.instagram && brandSocial.instagram.url">
              <a
                :href="brandSocial.instagram.url"
                target="_blank"
                rel="noopener"
    ...
    >
      <div
        class="max-w-7xl mx-auto px-4 text-center text-xs text-gray-500 dark:text-gray-500 tracking-wide"
      >
