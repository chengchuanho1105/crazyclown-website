...
<template>
  <nav v-if="!isHomePage && breadcrumbItems.length > 0" class="breadcrumb-container">
    ...
    <!-- 風格2: 極簡風格 -->
    <div
      v-else-if="currentStyle === 'minimal'"
      class="flex items-center space-x-1 px-4 py-2 text-sm"
    >
      <template v-for="(item, index) in breadcrumbItems" :key="item.path">
        <router-link
          v-if="!item.isLast"
    ...
    <!-- 風格4: 箭頭風格 -->
    <div
      v-else-if="currentStyle === 'arrows'"
      class="flex items-center px-4 py-3 bg-gray-50 dark:bg-gray-900"
    >
      <template v-for="(item, index) in breadcrumbItems" :key="item.path">
        <router-link
          v-if="!item.isLast"
    ...
    <!-- 風格6: 漸層風格 -->
    <div
      v-else-if="currentStyle === 'gradient'"
    ...
    >
      <template v-for="(item, index) in breadcrumbItems" :key="item.path">
        ...
        <div
          v-if="index < breadcrumbItems.length - 1"
          class="w-1 h-1 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full"
        ></div>
      </template>
    </div>
    ...
    <!-- 風格9: 底線風格 -->
    <div
      v-else-if="currentStyle === 'underline'"
    ...
    >
      <template v-for="(item, index) in breadcrumbItems" :key="item.path">
        ...
        <span
          v-else
          class="text-gray-900 dark:text-gray-100 font-semibold border-b-2 border-blue-500 pb-1"
        >
          {{ item.name }}
        </span>
        <svg
