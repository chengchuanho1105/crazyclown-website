...
const {
  ...
  error: newsDataError,
  load: loadNewsData
} = useSheetData<NewsDetailData>('https://docs.google.com/spreadsheets/d/e/2PACX-1vR7y16Qi2dNWRFl7OVgU78wv0SIMi_lPFt0WbZ6-7OqBFo7z2pN7LHs2heesTI4W5TnHM3lTcsXJS8s/pub?output=csv', (item: Record<string, string>) => {
  // 處理標籤欄位，移除多餘的引號
  let tags = item.tags || ''
...
<template>
  <div class="max-w-5xl mx-auto my-8 p-4 md:p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-lg">
    ...
    <div v-else-if="news">
      ...
      <div class="prose max-w-none text-gray-700 dark:text-gray-300 leading-relaxed" v-html="news.introduce"></div>
      <hr class="my-6 border-gray-200 dark:border-gray-700">

      <!-- 顯示詳細文章內容 -->
      <div v-if="news.html" class="mt-8 prose max-w-none text-gray-700 dark:text-gray-300 leading-relaxed">
