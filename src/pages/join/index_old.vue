...
const steamId = useFormField('', validators.steamId)
...
const handleSubmit = async (event: Event) => {
  ...
  if (isFormValid.value) {
    ...
    const applicationData = {
      ...
      steam_17_id: steamId.value.value as string,
      ...
      <form class="text-gray-900 dark:bg-zinc-900" autocomplete="off" @submit="handleSubmit">
        ...
        <div class="mb-8 p-6 bg-white rounded-lg shadow-md dark:bg-zinc-800 dark:shadow-lg">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <!-- Steam 17位數字ID -->
            <FormInput v-model="steamId.value.value" label="Steam 17位數字ID" :is-focused="steamId.isFocused.value"
              :status="steamId.status.value" name="Steam 17位數字ID" error-message="請輸入有效的 Steam 17位數字ID" maxlength="17"
              required @focus="steamId.handleFocus()" @blur="steamId.handleBlur()" />
