...
const loadFaqData = async () => {
  ...
  error.value = null
  try {
    faqList.value = await loadCsvData(FAQ_CSV_URL, mapFaqItem, processFaqData)
          ...
          <div class="flex flex-col gap-3">
            ...
            >
              <button
                ...
                :aria-expanded="openIndexes[list.category] === idx"
                @click="toggle(list.category, idx)"
              >
                <span>{{ item.question }}</span>
                <svg
                  class="ml-3 transition-transform duration-300"
                  :class="{ 'rotate-180': openIndexes[list.category] === idx }"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                >
              ...
              <div
                class="px-5 overflow-hidden transition-all duration-300"
