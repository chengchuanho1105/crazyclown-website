...
export function useFooterLinks() {
  ...
  const processRoutes = (
    ...
    return routes
      .filter((route) => {
        // 只顯示 layout: default 且 navbar 為 true 的路由
        const routeLayout = route.meta.layout || 'defa        ult'
        const navbarEnabled = route.meta.ui?.navbar === true

        return routeLayout === 'default' && navbarEnabled
      })
      .map((route) => {
        // 修正路徑組合邏輯
        let fullPath = ''
