...
const menuItems = [
  ...
  { name: '訂單管理', icon: 'bi-box', path: '/admin/order' },
  { name: '會員管理', icon: 'bi-people', path: '/admin/members' },
  { name: '商品管理', icon: 'bi-shop', path: '/admin/products' },
  { name: '系統設定', icon: 'bi-gear', path: '/admin/settings' },
]
</script>

<template>
  <div class="min-h-screen bg-amber-50 dark:bg-amber-950">
    <!-- 側邊欄 -->
    <aside
      :class="[
        'fixed inset-y-0 left-0 z-50 w-64 bg-amber-600 dark:bg-amber-800 shadow-lg transform transition-transform duration-300 ease-in-out',
      ...
      <div class="flex items-center justify-between p-4 border-b border-amber-500 dark:border-amber-700">
        <div class="flex items-center space-x-3">
          <div class="w-8 h-8 bg-amber-400 dark:bg-amber-600 rounded-lg flex items-center justify-center">
            <span class="text-amber-900 dark:text-amber-100 font-bold text-lg">A</span>
          </div>
          <h1 class="text-xl font-bold text-amber-100 dark:text-amber-200">Admin Panel</h1>
        </div>
      </div>
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-amber-500 dark:border-amber-700">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xs text-amber-200 dark:text-amber-300">admin@example.com</p>
          </div>
        </div>
      </div>
    </aside>
